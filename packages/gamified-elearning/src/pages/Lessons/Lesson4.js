...
const Lesson4 = () => {
  ...
  const handleCodeOutput = (output) => {
    ...
    if (nowEligible && !hasMarkedComplete) {
      ...
      trackStaticLessonCompletion(4).catch(err => console.error('Lesson 4 completion error:', err));
    }
  };
  ...
  const goToQuiz = async () => {
    ...
    if (!isLessonEligibleForCompletion()) {
      showLessonLockedToast(4);
      return;
    }

    if (!isCompleted) {
      try {
        const result = await trackStaticLessonCompletion(4);
    ...
    navigate('/quiz/4', { state: { source: 'lesson', lessonId: 4 } });
  };
